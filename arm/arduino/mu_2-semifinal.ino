...
int msg[3]; //массив приходящих данных
int msg2[2]; //массив данных подвеса
int x = 0;
int y = 0;
int p = 0;
...
void setup() {
  ...
  servoX.attach(4); // Servo присоединен к 4 выводу
  servoY.attach(7); // Servo присоединен к 7 выводу
...
void loop() {
  ...
  /*здесь код для моторов */
  
  p = msg[0];
  if (p == 0) { // проверка положения кнопки
    x = 90;
    y = 90;
    servoX.write(x); // Повернуть X-осный серво влево на х градусов
    servoY.write(y); // Повернуть Y-осный серво влево на у градусов
    delay(15); 
  }
  else {
    x = msg2[1] - msg[2] + 90; //это чёткий серв
    y = msg2[0] - msg[1] + 90; // это тоже неплохой серв
    servoX.write(x);  // Повернуть X-осный серво влево на х градусов
    delay(15);          // Пауза на пооворот. 
    servoY.write(y);  // Повернуть Y-осный серво влево на у градусов
    delay(15);          // Пауза на пооворот.   
  }
}
